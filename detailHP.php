...
if (isset($_POST['cari'])){
	$cari=$_POST['cari'];
	$sql="select * from dhakpakai where no_hak like'%$cari%' or
						  kel like '%$cari%' or
						  kec like '%$cari%'";
}else{
	$sql="select * from dhakpakai";		
}
...
if (isset($_POST['cari'])){
	...
	$sql="select * from dhakpakai limit $awalData,$jmlDataPerHal";		
}
...
<div class="jumbotron jumbotron-fluid bg-petanikode1 text-white">
            <div class="container">
                <div class="row">
                    <div class="col">
					<center>
							<h1 class="an">~ Buku Tanah J.Hak Pakai ~</h1>
						</center>
...
<div class="utama">
    <span class="float-left">
		<a class="btn btn-success" href="tambahDHP.php">Tambah Data</a>
	</span>
    <!--
	...
	if ($kosong){
		...
		while($row=mysqli_fetch_assoc($hasil)){
			...
			<tr>
				...
				<td>
				<a class="btn btn-outline-primary btn-sm" href="editDHP.php?kode=<?php echo $row['id']?>">Edit</a>
				<a class="btn btn-outline-danger btn-sm" href="hpsDHP.php?kode=<?php echo $row["id"]?>" id="linkHps" onclick="return confirm('Yakin dihapus nih?')">Hapus</a>
				</td>
			</tr>
			<?php 
			$no++;
