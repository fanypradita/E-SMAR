...
if (isset($_POST['cari'])){
	$cari=$_POST['cari'];
	$sql="select * from haktanggung where nh like'%$cari%' or
						  kel like '%$cari%' or
						  kec like '%$cari%'";
}else{
	$sql="select * from haktanggung";		
}
...
if (isset($_POST['cari'])){
	...
	$sql="select * from haktanggung limit $awalData,$jmlDataPerHal";		
}
...
<!--BEDA SECTION-->
	...
	$id=$_GET['kode'];
	$sql="select * from haktanggung where id='$id'";
	...
	<div class="container1">
		...
		<div class="row">	
		<div class="col-sm-9">
			<form enctype="multipart/form-data" method="post" action="simpanEHT.php">
				...
				<div class="form-group">
					...
					<input class="form-control" type="text" name="nh" id="nh" value="<?php echo $row['nh']?>" >
				</div>
				<div class="form-group">
                ...
                <div class="form-group">
					...
					<input class="form-control" type="text" name="rak" id="rak" value="<?php echo $row['rak']?>">
				</div>	
                <div class="form-group">
					<label for="bar">Baris:</label>
					<input class="form-control" type="number" name="bar" id="bar" value="<?php echo $row['bar']?>">
				</div>	
				<div class="form-group">
					<label for="ban">Bandel:</label>
					<input class="form-control" type="text" name="ban" id="ban" value="<?php echo $row['ban']?>">
				</div>			
				<div>		
	...
	<script>
		$('#submit').on('click',function(){
			...
			$.ajax({
				method	: "POST",
				url		: "simpanEHT.php",
				data	: {id:id, nh:nh, jns:jns, kec:kec, kel:kel, rak:rak, bar:bar ,ban:ban}
			});
		});
	</script>
</body>
