...
if (isset($_POST['cari'])){
	$cari=$_POST['cari'];
	$sql="select * from dhakwakaf where no_hak like'%$cari%' or
						  kel like '%$cari%' or
						  kec like '%$cari%'";
}else{
	$sql="select * from dhakwakaf";		
}
...
if (isset($_POST['cari'])){
	...
	$sql="select * from dhakwakaf limit $awalData,$jmlDataPerHal";		
}
...
<div class="jumbotron jumbotron-fluid bg-petanikode1 text-white">
            <div class="container">
                <div class="row">
                    <div class="col">
					<center>
							<h1 class="an">~ Buku Tanah J.Hak Wakaf ~</h1>
						</center>
...
<div class="utama">
    <span class="float-left">
		<a class="btn btn-success" href="tambahDHW.php">Tambah Data</a>
	</span>
    <!--
	...
	if ($kosong){
		...
		while($row=mysqli_fetch_assoc($hasil)){
			...
			<tr>
				...
				<td>
				<a class="btn btn-outline-primary btn-sm" href="editDHW.php?kode=<?php echo $row['id']?>">Edit</a>
				<a class="btn btn-outline-danger btn-sm" href="hpsDHW.php?kode=<?php echo $row["id"]?>" id="linkHps" onclick="return confirm('Yakin dihapus nih?')">Hapus</a>
				</td>
			</tr>
			<?php 
			$no++;
