...
if (isset($_POST['cari'])){
	$cari=$_POST['cari'];
	$sql="select * from hakmilik where id like'%$cari%' or
						  kel like '%$cari%' or
						  kec like '%$cari%'";
}else{
...
<div class="utama">
    <span class="float-left">
		<a class="btn btn-success" href="tambahHM.php">Tambah Data</a>
	</span>
    <!--
	...
	if ($kosong){
		...
		while($row=mysqli_fetch_assoc($hasil)){
			...
			<tr>
				...
				<td>
				<a class="btn btn-outline-primary btn-sm" href="detailHM.php?kode=<?php echo $row['id']?>">Detail</a>
				<a class="btn btn-outline-primary btn-sm" href="editHM.php?kode=<?php echo $row['id']?>">Edit</a>
				<a class="btn btn-outline-danger btn-sm" href="hpsHM.php?kode=<?php echo $row['id']?>" id="linkHps" onclick="return confirm('Yakin dihapus nih?')">Hapus</a>
				</td>
			</tr>
			<?php 
			$no++;
